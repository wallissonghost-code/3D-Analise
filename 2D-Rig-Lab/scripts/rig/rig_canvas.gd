...
func refresh_piece_texture(piece:Dictionary)->void:
	var path:String=piece.get("texture",""); textures.erase(path); _load_piece_texture(piece); queue_redraw()
...
func _draw_onion_skins()->void:
	var max_frames:=project.frame_count
	for offset in range(onion_previous,0,-1):
		var f:=int(round(onion_frame))-offset
		if f>=1:_draw_onion_frame(float(f),Color(0.35,0.75,1.0,onion_alpha/max(1.0,float(offset))))
	for offset in range(1,onion_next+1):
		var f:=int(round(onion_frame))+offset
		if f<=max_frames:_draw_onion_frame(float(f),Color(1.0,0.45,0.55,onion_alpha/max(1.0,float(offset))))
...
func _draw_onion_frame(frame_f:float,tint:Color)->void:
	...
	for kind in ["piece","bone","socket"]:
		...
		for item in arr:
			original.append({"item":item,"transform":_transform_dict(item)})
			var sample:=project.sample_track(item.get("id",""),frame_f)
			if not sample.is_empty():_apply_transform(item,sample)
	bone_cache.clear()
	for piece in _sorted_pieces():_draw_piece_ghost(piece,tint)
	for saved in original:_apply_transform(saved["item"],saved["transform"])
	bone_cache.clear()

func _draw_piece_ghost(piece:Dictionary,tint:Color)->void:
	var path:String=piece.get("texture","")
	if not textures.has(path):_load_piece_texture(piece)
	if not textures.has(path):return
	var tex:Texture2D=textures[path]; var world:Transform2D=RigSolverClass.piece_world(project,piece,bone_cache)
	var pivot:=_arr_v2(piece.get("pivot",[.5,.5])); var rs:=Vector2(tex.get_width(),tex.get_height()); var origin:=-Vector2(rs.x*pivot.x,rs.y*pivot.y)
	draw_set_transform(_world_to_screen(world.origin),world.get_rotation(),world.get_scale()*zoom); draw_texture(tex,origin,tint); draw_set_transform(Vector2.ZERO,0.0,Vector2.ONE)

func _draw_grid()->void:
	var spacing:=32.0*zoom
	if spacing<10.0:spacing*=4.0
...
func _sorted_pieces()->Array:
	var out:=project.pieces.duplicate(true);out.sort_custom(func(a,b):return int(a.get("z_index",0))<int(b.get("z_index",0)));return out
func _draw_piece(piece:Dictionary)->void:
	var path:String=piece.get("texture","")
	if not textures.has(path):_load_piece_texture(piece)
	if not textures.has(path):return
	var tex:Texture2D=textures[path];var world:Transform2D=RigSolverClass.piece_world(project,piece,bone_cache);var pivot:=_arr_v2(piece.get("pivot",[.5,.5]));var rs:=Vector2(tex.get_width(),tex.get_height());var origin:=-Vector2(rs.x*pivot.x,rs.y*pivot.y)
	draw_set_transform(_world_to_screen(world.origin),world.get_rotation(),world.get_scale()*zoom);draw_texture(tex,origin)
	if piece.get("id","")==selected_id:
		draw_rect(Rect2(origin,rs),Color("#57d9ff"),false,2.0/max(zoom,.1))
		if show_pivots:draw_circle(Vector2.ZERO,4.5/max(zoom,.1),Color("#ffcc4d"))
	draw_set_transform(Vector2.ZERO,0.0,Vector2.ONE)
func _draw_bones()->void:
	for bone in project.bones:
		var world:Transform2D=RigSolverClass.bone_world(project,bone.get("id",""),bone_cache);var a:=_world_to_screen(world.origin);var length:=float(bone.get("length",50.0))*world.get_scale().x*zoom;var b:=a+Vector2.RIGHT.rotated(world.get_rotation())*length;var col:=Color("#ffcc4d") if bone.get("id","")==selected_id else Color(1,.55,.25,.86);draw_line(a,b,col,4);draw_circle(a,5,col);draw_circle(b,4,col)
	...
	if event is InputEventMouseButton:
		if event.button_index==MOUSE_BUTTON_WHEEL_UP and event.pressed:_zoom_at(event.position,1.12);accept_event()
			...
			if event.pressed:
				...
				if not hit.is_empty():
					selected_id=hit.get("id","");dragging=hit.get("kind","")=="piece"
					if dragging:
						var piece:=_piece_by_id(selected_id);drag_start_mouse=event.position;drag_start_pos=_arr_v2(piece.get("position",[0,0]));drag_before=_transform_dict(piece)
					emit_signal("item_selected",hit.get("kind","piece"),selected_id)
				else:selected_id=""
				queue_redraw()
			else:
				if dragging:
					var piece:=_piece_by_id(selected_id);emit_signal("item_transform_changed",selected_id,drag_before,_transform_dict(piece))
				dragging=false
	if event is InputEventMouseMotion:
		if panning:pan+=event.relative;queue_redraw()
			...
			if not piece.is_empty():
				var dw:=(event.position-drag_start_mouse)/zoom;var parent:=_piece_parent_world(piece);piece["position"]=_v2_arr(drag_start_pos+parent.basis_xform_inv(dw));queue_redraw()
func _piece_parent_world(piece:Dictionary)->Transform2D:
	...
	if not socket_name.is_empty():
		var socket:=RigSolverClass.socket_by_name(project,socket_name)
		if not socket.is_empty():return RigSolverClass.socket_world(project,socket,bone_cache)
	return RigSolverClass.bone_world(project,piece.get("bone_parent",""),bone_cache)
func _zoom_at(mouse:Vector2,factor:float)->void:
	var before:=_screen_to_world(mouse);zoom=clamp(zoom*factor,.15,8.0);pan=mouse-before*zoom;queue_redraw()
func _hit_test(screen_pos:Vector2)->Dictionary:
	if project==null:return {}
	bone_cache.clear();var pieces:=_sorted_pieces();pieces.reverse()
	for piece in pieces:
		var path:String=piece.get("texture","")
		if not textures.has(path):continue
		var tex:Texture2D=textures[path];var wm:=_screen_to_world(screen_pos);var xf:Transform2D=RigSolverClass.piece_world(project,piece,bone_cache);var local:=xf.affine_inverse()*wm;var pivot:=_arr_v2(piece.get("pivot",[.5,.5]));var rect:=Rect2(-Vector2(tex.get_width()*pivot.x,tex.get_height()*pivot.y),Vector2(tex.get_width(),tex.get_height()))
		if rect.has_point(local):return {"kind":"piece","id":piece.get("id","")}
	for socket in project.sockets:
		var sw:=RigSolverClass.socket_world(project,socket,bone_cache)
		if _world_to_screen(sw.origin).distance_to(screen_pos)<=12:return {"kind":"socket","id":socket.get("id","")}
	for bone in project.bones:
		var bw:=RigSolverClass.bone_world(project,bone.get("id",""),bone_cache)
		if _world_to_screen(bw.origin).distance_to(screen_pos)<=12:return {"kind":"bone","id":bone.get("id","")}
	return {}
func _apply_transform(item:Dictionary,t:Dictionary)->void:
...
func _piece_by_id(id:String)->Dictionary:
	...
	return {}
func _bone_by_id(id:String)->Dictionary:
	for b in project.bones:
		if b.get("id","")==id:return b
	return {}
func _socket_by_id(id:String)->Dictionary:
