...
func build_ui() -> void:
	...
	toggle_button.text = "Skins & Equipamentos"
	toggle_button.pressed.connect(_toggle_mobile)
	root.add_child(toggle_button)
	content = VBoxContainer.new()
	content.add_theme_constant_override("separation", 6)
	root.add_child(content)
	var title := Label.new(); title.text = "SKINS & EQUIPAMENTOS"; content.add_child(title)
	info = Label.new(); info.text = "Skin troca PNG sem alterar bones ou animação."; info.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; content.add_child(info)
	var skin_label := Label.new(); skin_label.text = "Skin ativa"; content.add_child(skin_label)
	skin_select = OptionButton.new(); skin_select.item_selected.connect(_on_skin_selected); content.add_child(skin_select)
	var skin_row := HBoxContainer.new(); skin_name = LineEdit.new(); skin_name.placeholder_text = "Skin_Red"; skin_name.size_flags_horizontal = Control.SIZE_EXPAND_FILL; skin_row.add_child(skin_name); var create_skin := Button.new(); create_skin.text = "+ Skin"; create_skin.pressed.connect(_create_skin); skin_row.add_child(create_skin); content.add_child(skin_row)
	var replace_btn := Button.new(); replace_btn.text = "Trocar PNG da peça nesta skin"; replace_btn.pressed.connect(_choose_skin_texture); content.add_child(replace_btn)
	var clear_btn := Button.new(); clear_btn.text = "Usar PNG original nesta peça"; clear_btn.pressed.connect(_clear_skin_texture); content.add_child(clear_btn)
	content.add_child(HSeparator.new())
	var weapon_label := Label.new(); weapon_label.text = "Armas"; content.add_child(weapon_label)
	weapon_select = OptionButton.new(); content.add_child(weapon_select)
	weapon_name = LineEdit.new(); weapon_name.placeholder_text = "Nome: Sword / Gun / Staff"; content.add_child(weapon_name)
	weapon_socket = LineEdit.new(); weapon_socket.text = "RightHand"; weapon_socket.placeholder_text = "Socket: RightHand"; content.add_child(weapon_socket)
	var weapon_row := HBoxContainer.new(); var add_weapon := Button.new(); add_weapon.text = "+ Registrar PNG"; add_weapon.pressed.connect(_choose_weapon_texture); weapon_row.add_child(add_weapon); var equip := Button.new(); equip.text = "Equipar"; equip.pressed.connect(_equip_selected); weapon_row.add_child(equip); content.add_child(weapon_row)
	var unequip := Button.new(); unequip.text = "Desarmar"; unequip.pressed.connect(_unequip); content.add_child(unequip)
...
func _apply_responsive_layout() -> void:
	...
	if _mobile:
		...
		offset_top = -390 if _expanded else -54
...
func _create_skin() -> void:
	...
	if SkinManagerClass.create_skin(editor.project, name): editor.project.active_skin = name; skin_name.clear(); _status("Skin criada: " + name); refresh()
...
func _on_skin_texture_selected(path: String) -> void:
	var used := _project_asset_path(path); SkinManagerClass.set_piece_texture(editor.project, editor.project.active_skin, editor.selected_id, used); _status("PNG substituído apenas em " + editor.project.active_skin); refresh()
...
func _choose_weapon_texture() -> void: weapon_dialog.popup_centered_ratio(0.7)
func _on_weapon_texture_selected(path: String) -> void:
	var used := _project_asset_path(path); var name := weapon_name.text.strip_edges(); if name.is_empty(): name = path.get_file().get_basename(); var socket := weapon_socket.text.strip_edges(); if socket.is_empty(): socket = "RightHand"; var id := WeaponManagerClass.register_weapon(editor.project, {"name":name,"texture":used,"socket":socket,"offset":[0.0,0.0],"rotation":0.0,"scale":[1.0,1.0]}); editor.project.equipped_weapon = id; _status("Arma registrada e equipada: " + name); weapon_name.clear(); refresh()
func _equip_selected() -> void:
	if weapon_select.item_count == 0: return
	var id: String = weapon_select.get_item_metadata(weapon_select.selected); if WeaponManagerClass.equip(editor.project, id): _status("Equipamento atualizado"); refresh()
func _unequip() -> void: WeaponManagerClass.equip(editor.project, ""); _status("Arma removida do preview"); refresh()
