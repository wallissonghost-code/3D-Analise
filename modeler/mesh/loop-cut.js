...
export function loopCut(source,t=.5,edgeIndices=null){
 const g=editableGeometry(source),p=g.getAttribute('position'),k=THREE.MathUtils.clamp(t,.05,.95),edges=buildEdges(g);
 ...
 for(let f=0;f<p.count/3;f++){
  ...
  // Preserve winding of original triangle.
  if((i0===0&&i1===1)||(i0===1&&i1===2)||(i0===2&&i1===0)){push(out,v[i0],M,O);push(out,M,v[i1],O)}
  else {push(out,v[i1],M,O);push(out,M,v[i0],O)}
 }
 // No edge selection: global center cut, useful as a generic fallback.
 if(!selected){out.length=0;for(let f=0;f<p.count/3;f++){const [a,b,c]=faceVertices(g,f),ab=a.clone().lerp(b,k),ac=a.clone().lerp(c,k);push(out,a,ab,ac);push(out,ab,b,c);push(out,ab,c,ac)}}
