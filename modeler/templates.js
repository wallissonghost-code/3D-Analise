...
export function availableMounts(root,type='wheel'){const out=[];root.traverse(o=>{if(o.userData?.isMount&&o.userData.mountType===type&&!o.userData.occupied)out.push(o)});return out}
export function snapToMount(object,mount){if(!object||!mount)return false;const p=mount.getWorldPosition(new THREE.Vector3());const parent=object.parent;if(parent)parent.worldToLocal(p);object.position.copy(p);mount.userData.occupied=true;object.userData.mountUuid=mount.uuid;object.userData.mountName=mount.name;return true}
...
export function createFurnitureTemplate(){const root=new THREE.Group();root.name='Mobiliário';root.userData.template='furniture';const top=createPrimitive('cube','Tampo');top.scale.set(1.7,.12,.85);top.position.y=1.15;applyMaterial(top,{color:'#7b4d2c',roughness:.72});root.add(top);[[-1.4,.5,-.62],[1.4,.5,-.62],[-1.4,.5,.62],[1.4,.5,.62]].forEach((p,i)=>{const leg=createPrimitive('cube','Pé_'+(i+1));leg.scale.set(.12,.58,.12);leg.position.set(...p);applyMaterial(leg,{color:'#5a351e',roughness:.78});root.add(leg)});return root}
export function createTemplate(type){if(type==='car')return createCarTemplate();if(type==='person')return createPersonTemplate();if(type==='robot')return createRobotTemplate();if(type==='furniture')return createFurnitureTemplate();const root=new THREE.Group();root.name='Objeto';root.userData.template='object';return root}
